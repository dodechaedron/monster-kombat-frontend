...
const DailyRewardDialog = ({ setDialogOpen, days, setDays, setPoint, setAvailable, setUser }) => {
            ...
            <div className="bs-content">
                ...
                <ul className="bs-content-daily">
                    {Array(10).fill(1).map((value, index) => (
                        <li className={`${days > index ? 'is-current' : 'is-disabled'} v-popper--has-tooltip`} key={`daily-reward-${index}`}>
                ...
                <button className="bottom-sheet-button button button-primary button-large is-sticky" onClick={handleClaim}>
                    <span>Claim</span>
                </button>
            </div>
        </div>
