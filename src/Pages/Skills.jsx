...
function Skills() {
    const { user, point, level, setProfitPerHour, profitPerHour, dailyCombo, setUser } = useContext(PointContext);
    ...
    const fetchComboData = async () => {
        try {
            ...
            const combosData = combos.map(combo => {
                const existingCombo = user.combos.find(value => value._id == combo._id);
            ...
            setComboData(combosData.sort((a, b) => a.profitPerHour - b.profitPerHour || (b.side === COMBO_SIDE.POSITIVE) - (a.side === COMBO_SIDE.POSITIVE)));
        }
        catch (e) {
                ...
                <main className="main">
                    <div className="inner boost relative">
                        <div className='flex flex-row justify-center items-center'>
                            ...
                            <h1 className='flex-1 text-center text-white text-3xl my-4'>Learn new Skills</h1>
                        </div>
                        <div className='flex justify-center items-center'>
                            <img src="/images/icons/skills.png" />
                        </div>
                        <p className='text-center px-[10%] !my-6'>
                            Learn new skills and expand your abilities to increase your earnings.
                        </p>
                        <div className="skills-list mb-60">
                            {!loading ? comboData.map((value, index) => (
                                <div className={`upgrade-item p-2 w-1/3 ${value?.completed ? '!bg-[#004917]' : ''} ${!value?.requiredText ? 'border border-[#0EEC53]' : ''}`} key={`combo-list-${index}`} onClick={() => { if (value?.requiredText) { return false; } setDialogOpen('combo-box'); setSelectedCombo(value) }}>
                                    {value?.completed && <div className='icon absolute -right-1 -top-1'>
                                        <img src="/images/icons/success.svg" />
                                    </div>}
                                    {value?.requiredText && <div className='absolute w-[100%] h-[100%] bg-gray-900 bg-opacity-50 z-10 rounded-lg left-0 top-0'>
                                        <img src="/images/icons/locked.png" className='block mx-auto my-10'></img>
                                    </div>}
                                    ...
                                    <div className="upgrade-item-info">
                                        <div className="upgrade-item-title text-lg z-20">{value.name}</div>
                                        <div className="upgrade-item-profit py-8">
                                            ...
                                            <div className="upgrade-buy-stats-info">
                                                <div className="price">
                                                    ...
                                                    <div className="price-value !text-lg">+{toHumanString(value.profitPerHour)}</div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="upgrade-item-bottom">
                                        ...
                                        <div className="upgrade-item-detail">
                                            {!value?.requiredText ? <div className="price">
                                                <div className="price-image">
                                                    <img
                                                ...
                                                <div className="price-value !text-lg">{toHumanString(value.point)}</div>
                                            </div> : <div className="price"><p>{value?.requiredText}</p></div>}
                                        </div>
                                    </div>
                                </div>
                            )) : <Spinner />}
                    ...
                    {/* <div className='sticky w-full bottom-24'>
                        <button className='button button-primary button-large mx-auto w-[90%]'>
                            Claim
                        </button>
                    </div> */}
                </main>
                ...
                <div className="bottom-sheet" style={{ display: isDialogOpen ? 'flex' : 'none' }}>
                    ...
                    <div className="bottom-sheet-inner">
                        ...
                        <div className="bottom-sheet-scroll" >
                            {isDialogOpen && <ComboBoxDialog setDialogOpen={setDialogOpen} selectedCombo={selectedCombo} level={level} point={point} setProfitPerHour={setProfitPerHour} fetchDailyMatchs={() => { }} setUser={setUser} fetchComboData={fetchComboData} />}
                        </div>
                    </div>
                </div>
