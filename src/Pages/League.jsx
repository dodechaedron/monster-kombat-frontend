            ...
            try {
                ...
                LEVELS.map((value, index) => {
                    ...
                    const levelUsers = users
                        .filter((_user) => _user.level == value)
                        .sort((a, b) => (b.point > a.point ? 1 : -1));
            ...
            <div className="page">
                <main className="main">
                    <div className="inner league">
                        <div
                            className={`swiper swiper-fade swiper-initialized swiper-horizontal swiper-ios swiper-watch-progress league-slider is-${LEVEL_DATA[currentLevel].name}`}
                        >
                            <div
                            ...
                            >
                                <div
                                    key={`league-list-${currentLevel}`}
                                    className={`swiper-slide league-slide is-${LEVEL_DATA[currentLevel].name} swiper-slide-visible swiper-slide-fully-visible swiper-slide-active`}
                                    style={{
                                        width: `100%`,
                                ...
                                >
                                    <div className="league-item">
                                        <div
                                            className={`league-item-image is-${LEVEL_DATA[currentLevel].name}`}
                                        >
                                            <picture
                                                className={`is-hamster-image is-level-${currentLevel}`}
                                            >
                                                <source
                                                    srcSet={`/images/hamsters/${currentLevel}.png`}
                                                    type="image/png"
                                                />
                                                <img
                                                    className="img-responsive"
                                                    src={`/images/hamsters/${currentLevel}.png`}
                                                    alt="Monster Kombat"
                                                />
                                            </picture>
                                        </div>
                                    ...
                                    <div className="league-data">
                                        <div className="league-data-name">
                                            {LEVEL_DATA[currentLevel].name}
                                        </div>
                                    </div>
                                    <div className="league-numbers">
                                        <span>{`from ${LEVEL_DATA[currentLevel].point}`}</span>
                                    </div>
                                    {currentLevel == level && (
                                        <div className="flex flex-row items-center justify-between mt-3">
                                            <span className="font-bold">
                                                {LEVEL_DATA[currentLevel].point}
                                            </span>
                                            <span className="font-bold">
                                                {LEVEL_DATA[currentLevel + 1]?.point ?? " - "}
                                            </span>
                                        </div>
                                    )}
                                        ...
                                        {currentLevel == level ? (
                                            <div className="league-progress-outer">
                                                <div className="league-progress">
                                                    <div
                                                        className="max-w-full league-progress-bar"
                                                        style={{
                                                            width: `${((point - LEVEL_DATA[level].point) / (LEVEL_DATA[level + 1]?.point ?? LEVEL_DATA[level].point * 10 - LEVEL_DATA[level].point)) * 100}%`,
                                                        }}
                                                    />
                                                </div>
                                            </div>
                            ...
                            <div
                                ...
                                onClick={() => {
                                    setCurrentLevel((prev) =>
                                        Math.min(Math.max(prev - 1, 1), TOP_LEVEL)
                                    );
                                }}
                            />
                            <div
                        ...
                        {!isLoading ? (
                            <div
                                className={`league-users is-${LEVEL_DATA[currentLevel].name}`}
                            >
                                {userRank[currentLevel - 1]?.map((_user, index) =>
                                    _user._id != user._id ? (
                                        <div
                                            className={`league-user bg-[#15140C] !rounded-full`}
                                ...
                                {currentLevel == level && (
                                    <div className="league-user is-my">
                                        <div className="league-user-avatar">
                                            <div className="user-avatar">
                                                <picture>
                                                    <source
                                                        srcSet="/images/earn/doge.png"
                                                        type="image/webp"
                                                    />
                                        ...
                                        <div className="league-user-position">
                                            {userRank[user.level - 1]
                                                ?.map((value) => value._id)
                                                ?.indexOf(user._id) + 1}
                                        </div>
                                    </div>
